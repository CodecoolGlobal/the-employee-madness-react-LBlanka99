...
const Kittens = () => {
    ...
    useEffect(() => {
        fetch(`/api/kittens/${id}`).then(res => res.json()).then(res => setKittens(res));
        fetch(`/api/employees/${id}`).then(res => res.json()).then(res => setEmployee(res));
    }, [id]);
    ...
    return (
        <div>
            <table>
                ...
                <tbody>
                    {kittens && employee && kittens.map(kitten => (
                        <tr key={kitten._id}>
                            <td>{kitten.name}</td>
